...
class githubapi(object):
    ...
    def process(self):
        ...
        self.logger_test.info("no. of languages used:%s"%len(lang))
        stargazers=[s for s in self.repo.get_stargazers()]
        try:
          k=[stargazers[i].login for i in range(0,20)]
        except :
            k=stargazers
        self.logger_test.info("Total number of repo of the user:%s"%len(repos_apache))
...
if __name__=="__main__":
    ...
    parser.add_option('-p','--platformname', 
                    ...
                    default="github")
    parser.add_option('-g','--repousername', 
                    help='enter the user name of the public repository',
                    default="apache")
    parser.add_option('-r','--reponame', 
                    help='enter the name of the repo',
                    default="spark")
    ...
    t1=githubapi(str.lower(options.username),str.lower(options.platformname),str.lower(options.repousername),str.lower(options.reponame))
